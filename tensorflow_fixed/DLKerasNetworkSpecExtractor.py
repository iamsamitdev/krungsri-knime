...
class DLKerasNetworkSpecExtractor(object):
    ...
    def __init__(self, model):
        ...
        # tensors:
        # แก้ไข: เปลี่ยนจาก set() เป็น list เพื่อรองรับ TF 2.12+ ที่ Tensor เป็น unhashable
        self._visited_inputs = []
        self._visited_outputs = []   # FIX: was set()
    ...
    def _determine_dimension_order(self):
        ...
        if data_format == 'channels_first':   # FIX: was `is` (deprecated string comparison)
            return 'TCDHW'
        else:
            return 'TDHWC'
    ...
    def _extract_node_input_tensor_specs(self, layer, node_idx):
        ...
        for tensor_idx, input_tensor in enumerate(input_tensors):
            # FIX: ใช้ identity comparison (is) แทน equality (==) และ in operator
            if any(t is input_tensor for t in self._model_inputs) and not any(t is input_tensor for t in self._visited_inputs):
                self._visited_inputs.append(input_tensor)   # FIX: was .add()
                ...
                # FIX: ใช้ identity comparison แทน .index() ที่ใช้ == ซึ่ง unhashable ใน TF 2.12+
                input_idx = next(i for i, t in enumerate(self._model_inputs) if t is input_tensor)
                self._input_specs[input_idx] = tensor_spec
    ...
    def _extract_node_output_tensor_specs(self, layer, node_idx):
        ...
        for tensor_idx, output_tensor in enumerate(output_tensors):
            # FIX: ใช้ identity comparison (is) แทน equality (==) และ in operator
            if not any(t is output_tensor for t in self._visited_outputs):
                self._visited_outputs.append(output_tensor)   # FIX: was .add()
                tensor_shape = output_shapes[tensor_idx]
                tensor_spec = self._create_tensor_spec(layer, node_idx, tensor_idx, output_tensor,
                                                       tensor_shape, self._dimension_order)
                # FIX: ใช้ identity comparison แทน `in` operator ที่ใช้ == ซึ่ง unhashable ใน TF 2.12+
                if any(t is output_tensor for t in self._model_outputs):
                    # preserve order of output tensors
                    # FIX: ใช้ identity comparison แทน .index() ที่ใช้ ==
                    output_idx = next(i for i, t in enumerate(self._model_outputs) if t is output_tensor)
                    self._output_specs[output_idx] = tensor_spec
